...
class MinimaxSearch(BaseSearchAlgorithm):
    ...
    Mô phỏng: Người xếp lịch (MAX) chọn ca/phòng tối ưu cho một lớp học phần,
    đối đầu với Sự cố phòng học/GV bận (MIN) cố gắng làm giảm chất lượng lịch bằng cách khóa tài nguyên.
    """
    ...
    def search(
        ...
        **kwargs: Any
        ...
        for period_id, room_id in domain[:15]: # Giới hạn 15 ca/phòng để tránh bùng nổ tổ hợp
    ...
    def _min_value(self, timetable: Timetable, graph: TimetableProblem, depth: int) -> float:
        ...
        val = float('inf')
        # MIN mô phỏng sự cố: Thử khóa lần lượt các phòng học học phần này đang sử dụng
